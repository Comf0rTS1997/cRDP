...
/* Subclass rdpPointer to carry the converted ARGB buffer for the lifetime of the cached
 * pointer. Pointer_Set just emits the cached buffer to Java; Free releases it. */
typedef struct
{
	rdpPointer pointer;
	BYTE* argbData;
...
/* Cursor state codes consumed by LibFreeRDP.OnCursorState. Keep in sync with the Kotlin side. */
#define ANDROID_CURSOR_STATE_HIDDEN 0
#define ANDROID_CURSOR_STATE_DEFAULT 1

static void android_emit_cursor_state(rdpContext* context, jint state)
{
	if (!context || !context->instance)
		return;
	freerdp_callback("OnCursorState", "(JI)V", (jlong)context->instance, state);
}

static BOOL android_Pointer_New(rdpContext* context, rdpPointer* pointer)
{
	androidPointer* ap = (androidPointer*)pointer;
	...
	ap->argbSize = 4ULL * pointer->width * pointer->height;
	ap->argbData = winpr_aligned_malloc(ap->argbSize, 16);
	if (!ap->argbData)
		return FALSE;

	/* Android Bitmap.Config.ARGB_8888 maps to Skia kRGBA_8888 — i.e. bytes in memory are
	 * stored as R,G,B,A (matches what gdi_init uses for the frame buffer). PIXEL_FORMAT_RGBA32
	 * delivers exactly that. */
	if (!freerdp_image_copy_from_pointer_data(
	        ap->argbData, PIXEL_FORMAT_RGBA32, 0, 0, 0, pointer->width, pointer->height,
	        pointer->xorMaskData, pointer->lengthXorMask, pointer->andMaskData,
	        pointer->lengthAndMask, pointer->xorBpp, &context->gdi->palette))
	{
		winpr_aligned_free(ap->argbData);
		ap->argbData = NULL;
		ap->argbSize = 0;
		return FALSE;
	}
...
static BOOL android_Pointer_Set(rdpContext* context, rdpPointer* pointer)
{
	...
	if (!context->instance)
		return TRUE;
	if (!ap->argbData || ap->argbSize == 0 || pointer->width == 0 || pointer->height == 0)
	{
		/* No bitmap available — fall back to local default cursor. */
		android_emit_cursor_state(context, ANDROID_CURSOR_STATE_DEFAULT);
		return TRUE;
	}

	/* Build the jbyteArray on the attached env, then dispatch through freerdp_callback so
	 * the existing cached LibFreeRDP global ref is used. Calling FindClass directly on a
	 * freshly-attached native thread fails — the system class loader can't see app classes. */
	attached = jni_attach_thread(&env);
	if (!env)
		return TRUE;

	arr = (*env)->NewByteArray(env, (jsize)ap->argbSize);
	if (arr)
	{
		(*env)->SetByteArrayRegion(env, arr, 0, (jsize)ap->argbSize, (const jbyte*)ap->argbData);
		freerdp_callback("OnCursorBitmap", "(J[BIIII)V", (jlong)context->instance, arr,
		                 (jint)pointer->width, (jint)pointer->height,
		                 (jint)pointer->xPos, (jint)pointer->yPos);
...
static BOOL android_Pointer_SetPosition(rdpContext* context, UINT32 x, UINT32 y)
{
	...
	WINPR_UNUSED(x);
	WINPR_UNUSED(y);
	/* Position is driven by the trackpad layer; no-op here. */
	return TRUE;
}
...
{
	WINPR_ASSERT(context);
	android_emit_cursor_state(context, ANDROID_CURSOR_STATE_HIDDEN);
	return TRUE;
}
